...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const request = await signUp({ username: username.trim(), nickname: nickname.trim(), password });
      await request();
      setStep(3);
    } catch (error) {
  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      <div className="flex flex-col items-start bg-white p-8 border border-slate-300 gap-6 w-96">
        <img src={brandRow} alt="LandOm" className="h-8" />
        <span className="text-2xl font-bold text-slate-800">회원가입</span>
        ...
        {/* Step 1 */}
        {step === 1 && (
          <form className="flex flex-col gap-4 w-full" onSubmit={handleStep1}>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-sm font-normal text-slate-800">비밀번호 확인</span>
              <input
                type="password"
                className="p-2.5 rounded-sm bg-slate-100 text-sm placeholder:font-medium w-full"
            ...
            >
              <span className="text-sm font-semibold text-white">다음 단계</span>
            </button>
          </form>
        )}
        ...
        {step === 2 && (
          <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-1">
              <span className="text-sm font-normal text-slate-800">닉네임</span>
              <input
                type="text"
                className="p-2.5 rounded-sm bg-slate-100 text-sm placeholder:font-medium w-full"
                placeholder="닉네임 입력"
            ...
            <div className="flex gap-2 mt-2">
              ...
              >
                <span className="text-sm font-semibold text-slate-600">이전</span>
              </button>
              <button
                type="submit"
                disabled={isLoading}
        ...
        {step === 3 && (
          <div className="flex flex-col gap-6 w-full">
            ...
            <button
              ...
              className="flex justify-center p-3 bg-blue-500 rounded-2xl w-full"
            >
              <span className="text-sm font-semibold text-white">로그인 하기</span>
            </button>
          </div>
        )}
      </div>
